...
export const status_last_updated_handler = (app: Probot) => {
  app.on(["projects_v2_item.edited", "projects_v2_item.created"], async (context) => {
    ...
    if (!(Object.keys(config)).includes(context.payload.projects_v2_item.project_node_id)) {
    ...
    try {
      ...
      const params = {
        project_id: context.payload.projects_v2_item.project_node_id,
        project_item_id: context.payload.projects_v2_item.node_id,
        date_field_id: config[context.payload.projects_v2_item.project_node_id],
        value: updatedAt,
      };
