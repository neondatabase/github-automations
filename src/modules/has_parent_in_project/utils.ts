...
export async function setFieldForProjectItem(context: Context<EmitterWebhookEventName>, projectItem: {
  id: string;
  project: { id: string }
}, fieldId: string) {
  try {
...
export async function clearFieldForProjectItem(context: Context<EmitterWebhookEventName>, projectItem: {
  id: string;
  project: { id: string; title: string }
}, fieldId: string) {
  try {
    context.log.info(`Try clear hasParentInProject value to ${HAS_PARENT_IN_PROJECT_VALUE} for item ${projectItem.id} in project ${projectItem.project.id}`)
    if (!isDryRun()) {
      ...
      context.log.info(`Done clear hasParentInProject value to ${HAS_PARENT_IN_PROJECT_VALUE} for item ${projectItem.id} in project ${projectItem.project.id} with res`, res);
    }
  } catch (e) {
    context.log.error(e);
  }
